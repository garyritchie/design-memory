...
function generateDefaultHierarchy(ir: DesignIR): string {
  ...
  if (ir.components.length > 0)
    items.push('Component consistency ensures cohesive user experience');
  if (ir.spacing.length > 0)
    items.push('Spacing system maintains visual rhythm and breathing room');
...
function generateDefaultConstraints(ir: DesignIR): string {
  ...
  if (ir.typography.length > 0)
    items.push('Do not use font sizes or weights not in the typography system');
...
function generateDefaultAntiPatterns(ir: DesignIR): string {
  ...
  if (ir.colors.length > 0)
    items.push('Avoid using colors that conflict with the established palette');
