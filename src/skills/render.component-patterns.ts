...
`);
  ...
  for (const comp of ir.components) {
    ...
    // Generate code snippet from extracted styles
    ...
    if (comp.do.length > 0) {
      sections.push(`**Do:** ${comp.do.join(' | ')}`);
    }
    if (comp.dont.length > 0) {
      sections.push(`**Don't:** ${comp.dont.join(' | ')}`);
    }
    if (comp.constraints.length > 0) {
      sections.push(`**Constraints:** ${comp.constraints.join(' | ')}`);
    }
    sections.push('');
  }
