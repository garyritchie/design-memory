...
export function detectComponents(styles: ComputedStyle[]): ComponentRecipe[] {
  ...
  for (const style of styles) {
    ...
    // Card: has shadow + border-radius + padding (usually div or section)
    if ((tag === 'div' || tag === 'section' || tag === 'article') && isCard(p)) {
      components.push(recipe('card', `${tag}-card`, p, 'Content card container'));
    }

    // Navigation: nav tag or header with horizontal layout
    if (tag === 'nav') {
    ...
    // Form: form tag or fieldset-like structure
    if (tag === 'form') {
  ...
  // Hero: large heading (h1) in a section/main with big padding
  ...
  // Badge/tag: span with small font + border-radius + background color
  ...
  // Form group: label + input pair detection
